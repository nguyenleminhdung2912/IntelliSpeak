...
//    Accounts | Auth
    ...
    TOKEN_INVALID(1006, "Token không hợp lệ", HttpStatus.BAD_REQUEST),
    TOKEN_EXPIRED(1021, "Token đã hết hạn", HttpStatus.BAD_REQUEST),
    ACCOUNT_NOT_VERIFY(1007, "Tài khoản chưa được xác minh", HttpStatus.BAD_REQUEST),
    INTERNAL_SERVER_ERROR(1008, "Internal Server Error", HttpStatus.INTERNAL_SERVER_ERROR),
    OLD_PASSWORD_INCORRECT(1009, "Mật khẩu cũ không đúng", HttpStatus.BAD_REQUEST),
    PASSWORD_REPEAT_INCORRECT(1010, "Mật khẩu nhập lại không khớp", HttpStatus.BAD_REQUEST),
    NOT_LOGIN(401, "Bạn cần đăng nhập", HttpStatus.BAD_REQUEST),
    USERNAME_PASSWORD_NOT_CORRECT(1012, "Email hoặc mật khẩu không đúng", HttpStatus.BAD_REQUEST),
    ACCOUNT_NOT_FOUND(404,"Không tìm thấy tài khoản", HttpStatus.NOT_FOUND),
    EMAIL_NOT_FOUND(1013,"Không tìm thấy email, vui lòng đăng ký", HttpStatus.NOT_FOUND),
    ...
    ACCOUNT_IS_DELETED(1016,"Tài khoản này đã bị xoá", HttpStatus.BAD_REQUEST),
...
//    Accounts | Emails | CODE: 15XX
    ...
    EMAIL_WAIT_VERIFY(1501, "Email đã được đăng ký, vui lòng xác minh và đăng nhập", HttpStatus.BAD_REQUEST),
